...
//Function for storing data from a files//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void displayMainMenu() {
	...
	cout << "[6] : Remove A Book From Collection//FIX ME" << endl;
	cout << "[7] : Remove A Member//FIX ME" << endl;
...
void removeMem(vector<Book> BookList, vector<Member> &MemberList) {
	...
	if (choiceDel < 1 || choiceDel > MemberList.size()) {
		...
		outFileUpdate << MemberList[MemberList.size() - 1].Name << endl;
		outFileUpdate << MemberList[MemberList.size() - 1].Code << endl;
		outFileUpdate.close();
	}
...
void removeBook(vector<Book> &BookList, vector<Member> MemberList) {
	...
	if (choiceDel < 1 || choiceDel > BookList.size()) {
		...
		outFileBkUpdate << BookList[BookList.size() - 1].Title << endl;
		outFileBkUpdate << BookList[BookList.size() - 1].Author << endl;
		outFileBkUpdate << BookList[BookList.size() - 1].Year << endl;
		outFileBkUpdate << BookList[BookList.size() - 1].Genre << endl;
		outFileBkUpdate << BookList[BookList.size() - 1].ISBN << endl;
...
/*bool checkOut(vector<Book> BookList, vector<Member> MemberList) {
	bool action;
	int
		s1 = 10,
		s2 = 25,
	...
	cout << endl;
	for (int i = 0; i < BookList.size(); i++) {
		cout << setw(s2) << "" << "[" << (i + 1) << "] : " << setw(s1) << BookList[i].Title << MemberList[i].Code << endl;
		cout << setw(s2) << "" << setw(s1) << "Title: " << BookList[i].Title << endl;
		cout << setw(s2) << "" << setw(s1) << "Author: " << BookList[i].Author << endl;
		cout << setw(s2) << "" << setw(s1) << "Year: " << BookList[i].Year << endl;
		cout << setw(s2) << "" << setw(s1) << "Genre: " << BookList[i].Genre << endl;
...
void checIOStat(vector<Book> BookList, vector<Member> MemberList, bool action) {
	int menuSelect;
